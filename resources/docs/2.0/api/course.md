...
- 参数：
  ...
  ```
- 说明：
  - 没有参数时，表示对当前用户执行加入课程的操作。
  - 普通用户可以进行无参数的操作，即自己加入课程。
  - 当`user_id`存在时，要求用户为课程管理员。
  - 当`type_is_admin`为存在时，要求用户为网站管理员。
- 返回：`200 OK` 加入课程的记录
  ```json
  enroll_record: {
        id: 1,
...
- 说明：
  ...
  - 当`user_id`存在时，要求用户已注册为课程管理员且操作对象不是课程管理员。
  - 不允许注册成为课程管理员的用户自己退出课程。
- 返回：`204 No Content`
