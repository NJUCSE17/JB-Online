...
class PersonalAssignmentTest extends TestCase
{
    ...
    public function testPersonalAssignmentFunctions()
    {
        ...
        for ($i = 0; $i < 2; ++$i) {
            ...
            $personal_assignments[$i]['content_html'] = $this->parser->text(
                $personal_assignments[$i]['content']
            );
        }
        ...
        $personal_assignments[1]['content_html'] = $this->parser->text(
            $personal_assignments[1]['content']
        );
        $personal_assignments[1]['due_time'] = $this->faker->dateTimeBetween(
            'now',
            '+5 days'
        )->format('Y-m-d H:i:s');
        if (true) {
